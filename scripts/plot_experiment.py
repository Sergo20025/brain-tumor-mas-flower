...
def build_round_dataframe(round_metrics_path: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    client_df = pd.DataFrame(clients).sort_values(["client_label", "round"]).reset_index(drop=True)
...
def plot_f1_dynamics(
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.lineplot(data=round_df, x="round", y="train_f1", marker="o", label="Train F1-macro", ax=ax)
...
def plot_data_volume_distribution(volume_df: pd.DataFrame, output_dir: Path) -> None:
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.barplot(data=volume_df, x="client", y="num_samples", color="#4C78A8", ax=ax)
    ax.set_title("Distribution of Training Data Volume Across Clients (non-IID)")
    ax.set_xlabel("Client")
    ax.set_ylabel("Number of Training Samples")
    ax.grid(True, axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(output_dir / "client_data_volume_non_iid.png", dpi=200)
    plt.close(fig)


def plot_class_distribution(class_df: pd.DataFrame, output_dir: Path, alpha: float) -> None:
    ...
    ax.set_title(f"Class Distribution Across Clients (non-IID, alpha={alpha})")
    ax.set_xlabel("Class")
    ax.set_ylabel("Client")
    fig.tight_layout()
    fig.savefig(output_dir / "class_distribution_non_iid_alpha_0_5.png", dpi=200)
    plt.close(fig)
...
def main() -> None:
    ...
    round_df, client_df = build_round_dataframe(Path(args.round_metrics))
    global_eval_df = build_global_eval_dataframe(Path(args.global_metrics))
    ...
    plot_data_volume_distribution(volume_df, output_dir)
    plot_class_distribution(class_df, output_dir, args.alpha)
